...
**Split long or multi-cut sequences into separate generations.** One generation per shot, joined afterwards in an editor, beats one long generation with cuts written into it. Every extra shot inside a single render costs coherence, and frames drive VRAM and time directly, so a 15-second multi-cut sequence is both the most expensive and the least reliable way to ask. Prefer splitting whenever the sequence has more than about two cuts, whenever it runs past roughly eight seconds, or whenever a beat needs a length very different from its neighbours. A **locked-off camera makes the joins invisible**, so a static sequence can be cut into as many pieces as convenient at no cost. Where the framing must match exactly across a join, generate the next clip as I2VA from the previous clip's final frame. Black frames, hard cuts and any transition are cheaper and cleaner added in the editor than generated.

**Commit to one exact length. Never offer a range.** This is an automated pipeline: the user should paste a number, not weigh options. Forbidden: `about 3–4 seconds`, `roughly 5 s`, `either 124 or 141`, `3.5 s or so`. Required: a single grid value with the duration it actually yields — `length 90 (3.75 s)`.
...
**Label every prompt in a split sequence.** When a sequence is delivered as several prompts, put a header line above each code block naming the prompt number, the reference images that prompt needs, and its length — `Prompt 2 — <Picture 1>, <Picture 3> — length 90 (3.75 s)`. The user is wiring different references per clip and setting a different length each time; making them reconstruct that from the prompt body is where mistakes happen.
...
**Find the through-line before writing the panels.** A strip is usually one continuous movement sampled at intervals, not six independent poses. State what travels and in which direction, then write the panels as waypoints on that path — six equal-weight poses can be reordered into any activity, a stated trajectory cannot. Naming the single travelling element and stating that the rest of the body stays quiet blocks the wrong activity by construction.

**Write a motion strip panel by panel, and count your motion words.** Enumerate the panels in order, one sentence each, using ordinals — *she begins … next … then … finally* — because the model cannot count and prose second-counts schedule nothing. And weigh the vocabulary: repeated ambient movement (bouncing, shifting weight, moving to the beat, mouthing along) outcompetes a key action named once at the end, and the render comes back as dancing. Name incidental motion once in a subordinate clause; give the key action a sentence in every panel it appears in. Crop burned-in captions off the strip rather than forbidding them in `retention_analysis` — repeated across every panel, they are its most prominent content.
...
**Frame count snaps to a 17k+5 grid**, and anything off it is rounded up silently — multiples of 4 are *not* the rule. The node accepts `5` to `3600`; **the minimum is five *frames*, about a fifth of a second, not five seconds.** Valid values at 24 fps:

`5` = 0.21 s · `22` = 0.92 s · `39` = 1.63 s · `56` = 2.33 s · `73` = 3.04 s · `90` = 3.75 s · `107` = 4.46 s · `124` = 5.17 s · `141` = 5.88 s · `158` = 6.58 s · `175` = 7.29 s · `192` = 8.00 s · `209` = 8.71 s · `226` = 9.42 s · `243` = 10.13 s · `260` = 10.83 s · `277` = 11.54 s · `294` = 12.25 s · `311` = 12.96 s · `328` = 13.67 s · `345` = 14.38 s · `362` = 15.08 s

The tooltip calls **~124–362** the trained range and marks longer as untested. It says nothing against shorter: values below 124 are accepted and simply unproven, not forbidden. A one- or two-second beat is a legitimate choice — do not tell the user 124 is a floor.
